  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    if (jsonObj == null) {
      if (Argent.openapiRequiredFields.isEmpty()) {
        ...
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in Argent is not found in the empty JSON string",
                Argent.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Entry<String, JsonElement> entry : entries) {
      if (!Argent.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `Argent` properties. JSON:"
                    + " %s",
