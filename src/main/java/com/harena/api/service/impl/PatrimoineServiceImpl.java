...
public class PatrimoineServiceImpl implements PatrimoineService {
  ...
  public PatrimoineServiceImpl(
      PatrimoineRepository patrimoineRepository, PersonRepository personRepository) {
  ...
  public List<PatrimoineSummarized> findAllPatrimoines(Integer page, Integer pageSize) {
    ...
    var patrimoines =
        patrimoineRepository.loadAllData().stream()
  ...
  public PatrimoineSummarized findPatrimoineByNom(String patrimoineName) {
    var patrimoineDataList = patrimoineRepository.loadAllData();
    var personMap =
        personRepository.loadAllData().stream()
            .collect(Collectors.toMap(PersonneDataJsonFile::getNom, Function.identity()));
    var patrimoineData =
        patrimoineDataList.stream()
  ...
  private PersonneDataJsonFile getOrCreatePerson(
      Map<String, PersonneDataJsonFile> personMap, String personName) {
    return personMap.computeIfAbsent(
        personName,
        name -> {
  ...
  private List<PatrimoineSummarized> getPagedResult(
      List<PatrimoineSummarized> patrimoines, Integer page, Integer pageSize) {
  ...
  private PatrimoineSummarized convertToSummarized(
      PatrimoineDataJsonFile patrimoineData, Map<String, PersonneDataJsonFile> personMap) {
    if (patrimoineData == null) {
      return null;
    }
    var possesseur =
        Optional.ofNullable(personMap.get(patrimoineData.getPossesseurNom()))
