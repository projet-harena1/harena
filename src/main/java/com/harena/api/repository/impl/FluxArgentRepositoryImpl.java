...
@Repository
public class FluxArgentRepositoryImpl extends BaseRepository<FluxArgentDTO>
    implements FluxArgentRepository {
  ...
  public List<FluxArgent> loadAllData() {
    ...
    super.loadAllData(FluxArgentDTO.class)
        .forEach(fluxArgentDTO -> flux.add(this.toFluxArgent(fluxArgentDTO)));
    return flux;
  }
  ...
  private FluxArgentDTO toFluxArgentDTO(FluxArgent fluxArgent) {
    var argentNom =
        argentRepository.findArgentByNomAndPatrimoine(
            fluxArgent.getNom(), fluxArgent.getPatrimoine().nom());
    ...
    if (devise != null) {
      ...
      if (foundDevise == null) {
        devise =
            deviseRepository
                .create(new Devise(devise.nom(), devise.code()))
  ...
  private FluxArgent toFluxArgent(FluxArgentDTO dto) {
    var patrimoine =
        (dto.getPatrimoineNom() != null)
            ? patrimoineRepository.findPatrimoineByNom(dto.getPatrimoineNom())
            : null;
    var deviseDTO = dto.getDevise();
    var devise = deviseDTO != null ? new Devise(deviseDTO.getNom(), deviseDTO.getCode()) : null;
  ...
  public FluxArgent findFluxArgentByNom(String nom) {
    return this.loadAllData().stream()
        .filter(fluxArgent -> fluxArgent.getNom().equals(nom))
        .findFirst()
        .orElse(null);
  }
