...
@Repository
public class MaterialRepositoryImpl extends BaseRepository<MaterielDTO>
  ...
  public List<Materiel> loadAllData() {
    ...
    super.loadAllData(MaterielDTO.class)
        .forEach(materielDTO -> materiels.add(this.toMateriel(materielDTO)));
    return materiels;
  }
  ...
  private MaterielDTO toMaterielDTO(Materiel materiel) {
    ...
    var devise =
        (materiel.getDevise() != null)
            ? new DeviseDTO(materiel.getDevise().nom(), materiel.getDevise().code())
            : null;
  ...
  private Materiel toMateriel(MaterielDTO materielDTO) {
    var patrimoine =
        (materielDTO.getPatrimoineNom() != null)
    ...
    if (materielDTO.getDevise() != null) {
      ...
      if (devise == null) {
        var createdDevise =
            deviseRepository.create(new Devise(materielDTO.getDevise().getNom(), code));
  ...
  public Materiel findMaterielByNom(String nom) {
    return this.loadAllData().stream()
        .filter(materiel -> materiel.getNom().equals(nom))
        .findFirst()
        .orElse(null);
  }
