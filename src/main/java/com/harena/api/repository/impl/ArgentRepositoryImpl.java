...
public class ArgentRepositoryImpl extends BaseRepository<ArgentDTO> implements ArgentRepository {
  ...
  public Argent findArgentByNomAndPatrimoine(String argentNom, String patrimoineNom) {
    return loadAllData().stream()
        .filter(
            argent ->
                argent.getNom().equals(argentNom)
                    && argent.getPatrimoine().nom().equals(patrimoineNom))
        .findFirst()
        .orElse(null);
  }
  ...
  public Argent findArgentByNom(String nom) {
    return this.loadAllData().stream()
        .filter(argent -> argent.getNom().equals(nom))
        .findFirst()
        .orElse(null);
  }
              ...
              .orElseThrow(
                  () ->
                      new IllegalStateException(
                          "Failed to create Devise with code: " + devise.code()));
  ...
  private Argent toArgent(ArgentDTO argentDTO) {
    var patrimoine =
        (argentDTO.getPatrimoineNom() != null)
