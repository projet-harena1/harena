...
public class ReadDataFromJsonFile<T> implements Function<Class<T>, Function<Path, List<T>>> {
  ...
  public ReadDataFromJsonFile(@Qualifier("customObjectMapper") ObjectMapper objectMapper) {
  ...
  private List<T> readFromFile(Class<T> tClass, Path path) {
    ...
    @Cleanup var inputStream = Files.newInputStream(path);
    var collectionType = objectMapper.getTypeFactory().constructCollectionType(List.class, tClass);
